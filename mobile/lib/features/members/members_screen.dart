...
final membersQueryProvider = StateProvider<String>((ref) => '');

final membersProvider = FutureProvider.autoDispose((ref) {
  final query = ref.watch(membersQueryProvider);
  final status = ref.watch(membersFilterProvider);
  return ref.watch(repositoryProvider).getMembers(query: query, status: status);
});

class MembersScreen extends ConsumerWidget {
  const MembersScreen({super.key, this.forCollection = false});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(forCollection ? 'Collection' : 'Members'),
        actions: [
          if (!forCollection)
            IconButton(
              tooltip: 'Add Member',
      ...
      floatingActionButton: forCollection
          ? null
      ...
      body: Column(
        children: [
          Padding(
            ...
            child: TextField(
              onChanged: (v) => ref.read(membersQueryProvider.notifier).state = v,
          ...
          Expanded(
            child: membersAsync.when(
              ...
              data: (members) {
                if (members.isEmpty) {
                  return const EmptyState(message: 'No members found');
                }
